<script lang="ts">
	let {
		...
		cached,
		href
	}: { name: string; isFolder: boolean; cached?: boolean; href: string } = $props();
...
<a
	{href}
	class="group relative flex flex-col items-center gap-2 rounded-md p-3 transition hover:bg-bg-hover"
>
	<div class="relative flex h-12 w-12 items-center justify-center text-accent-dim">
		{#if isFolder}
			<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.2" class="h-10 w-10">
		...
		{#if cached}
			<span
				class="absolute right-0 top-0 h-2 w-2 rounded-full bg-accent shadow-[0_0_4px_var(--color-accent)]"
				title="cached"
			></span>
		{/if}
	</div>
	<span class="line-clamp-2 text-center text-[11px] leading-tight text-text-primary">{name}</span>
</a>
